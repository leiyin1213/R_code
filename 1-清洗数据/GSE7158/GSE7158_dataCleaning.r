...
GSE <- "GSE7158"  #数据集名称
GPL<- "GPL570-55999.txt" #芯片名称
geneName<-"Gene.Symbol" #芯片对应的基因行名
base_path <- "G:/Rcode/实验数据/实验数据/1-清洗数据"  #数据存放路径
full_path <- file.path(base_path, GSE)
setwd(full_path)

# ===== 1) download GEO =====

gset <- getGEO(GSE,destdir = full_path,AnnotGPL= F, getGPL= F)
...
#获取基因symbol
library(stringr)
ids$Symbol= trimws(str_split(ids$`Symbol`,"///",simplify= T)[,1]) #根据符号进行分割(需要按照芯片数据修改)
#去除没有注释的探针
ids= ids[ids$Symbol !='',]
ids= ids[ids$Symbol !='---',]
ids= ids[ids$Symbol !='---',]
